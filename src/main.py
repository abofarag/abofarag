...
# e.g., "from .telegram_bot" instead of "from src.telegram_bot"
try:
    from .ai_agent import AIAgent
    from .sheets_manager import GoogleSheetsManager
    from .manychat import ManyChatAPI
    from . import config
    from .telegram_bot import main as run_telegram_bot_async
except ImportError as e:
    logger.error(f"FATAL: A module import failed. This can happen if the file structure is incorrect. Error: {e}")
    # This will stop the app if a critical module is missing.
    raise
...
@app.get("/")
def read_root():
    return {"Status": "AI Instagram Bot Controller is running"}

# --- Uvicorn Runner for local development ---
if __name__ == "__main__":
    port = int(os.getenv("PORT", 8000))
    logger.info(f"Starting Uvicorn server on host 0.0.0.0, port {port}")
    # Note: For local dev, you might need to adjust run command depending on your project structure
